...
if($per=="1"){
	...
	$active=0;
		...
		$sql="update `auth_user` set `user` ='{$user}',`pass` ='{$pass}',`dlqq` ='{$dlqq}',`per_sq` ='{$per_sq}',`per_db` ='{$per_db}',`active` ='{$active}' where `uid`='{$uid}'";
        ...
        <div class="panel-body">
		  ...
		  <input type="hidden" name="backurl" value="<?php echo $_SERVER['HTTP_REFERER']; ?>"/>
			...
			<div class="form-group">
              ...
              <div class="col-sm-10"><input type="text" name="dlqq" value="<?php echo $row['qq']; ?>" class="form-control" ></div>
            </div><br/>
            <?php } else{?>
            	<div class="form-group">
              <label class="col-sm-2 control-label">密码</label>
              <div class="col-sm-10"><input type="password" name="pass" value="" class="form-control" disabled/></div>
            </div><br/><?php } ?>
			<div class="form-group">
              ...
              <div class="col-sm-10"><select name="per" class="form-control">
				...
				if(($udata['uid'])=="1"){$all='<option value="4">全部操作权限</option>"';}else{$all='';}
				if(($row['active'])=="0"){
				
					echo '
					<option value="3">该用户已封禁</option>
					<option value="1">授权操作权限</option>
					<option value="2">获取操作权限</option>
						'.$all;
