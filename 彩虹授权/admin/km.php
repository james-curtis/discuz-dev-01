...
echo '<a href="./km.php" class="btn btn-default btn-block">>>返回卡密列表</a>';
}
...
echo '<hr/><a href="./km.php">>>返回卡密列表</a></div></div>';
}
...
<div class="panel-body box">
您确认要清空所有卡密吗？清空后无法恢复！<br><a href="./km.php?my=qk2">确认</a> | <a href="javascript:history.back();">返回</a></div></div>';
}
...
<div class="panel-body box">
您确认要清空所有已使用过的卡密吗？清空后无法恢复！<br><a href="./km.php?my=qkuse2">确认</a> | <a href="javascript:history.back();">返回</a></div></div>';
}
elseif($my=='qkuse2'){//清空已使用卡密结果
echo '<div class="panel panel-primary">
<div class="panel-heading w h"><h3 class="panel-title">清空已使用卡密</h3></div>
<div class="panel-body box">';
if($DB->query("DELETE FROM auth_kms WHERE zt='1'")==true){
echo '<div class="box">清空成功.</div>';
}else{
echo'<div class="box">清空失败.</div>';
}
...
echo '<form action="km.php?my=add" method="POST" class="form-inline">
  <div class="form-group">
    <label>卡密生成</label>
    <input type="text" class="form-control" name="num" placeholder="生成的个数" required>
  </div>
  <div class="form-group">
    <input type="text" class="form-control" name="value" placeholder="生成的余额">
  </div>
  <button type="submit" class="btn btn-primary">生成</button>
  <a href="km.php?my=qk" class="btn btn-danger">清空</a>
  <a href="km.php?my=qkuse" class="btn btn-danger">清空已使用</a>
</form>';
...
if(isset($_GET['kw'])) {
	...
	$numrows=$DB->count("SELECT count(*) from auth_kms WHERE 1");
	$sql=" 1";
...
?>
      <div class="table-responsive">
        <table class="table table-striped">
          <thead><tr><th>卡密</th><th>面值</th><th>状态</th><th>添加时间</th><th>使用时间</th><th>操作</th></tr></thead>
          <tbody>
<?php
$pagesize=30;
...
{
if($res['zt']==1) {
	$zt='<font color="red">已使用</font>&nbsp;使用者:'.$user['user'];
} elseif($res['zt']==0) {
	$zt='<font color="green">未使用</font>';
}
echo '<tr><td><b>'.$res['km'].'</b></td><td>'.$res['value'].'</td><td>'.$zt.'</td><td>'.$res['addtime'].'</td><td>'.$res['usetime'].'</td><td><a href="./km.php?my=del&id='.$res['id'].'" class="btn btn-xs btn-danger" onclick="return confirm(\'你确实要删除此卡密吗？\');">删除</a></td></tr>';
}
?>
          </tbody>
        </table>
...
echo '<li><a href="km.php?page='.$prev.$link.'">&laquo;</a></li>';
} else {
...
if ($page<$pages)
{
echo '<li><a href="km.php?page='.$next.$link.'">&raquo;</a></li>';
