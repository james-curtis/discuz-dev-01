...
if(!defined('IN_DISCUZ') || !defined('IN_ADMINCP')) {
	...
	if($allcount){
		...
		foreach($query as $val){
			...
			if ($type == 1)
            {
                $type = lang('plugin/keke_group', 'lang12');
            }
            elseif ($type == 2)
            {
                $type = lang('plugin/keke_group', 'lang13');
            }
            else{
                if (file_exists(DISCUZ_ROOT.'./source/plugin/dc_pay/payment.lib.class.php'))
                {
                    require_once DISCUZ_ROOT.'./source/plugin/keke_group/xhw6.class.php';
                    $pay = new xhw6();
                    if (in_array($type,$pay->getPayType()))
                    {
                        $type = $pay->getAllPayInfo()[$type]['title'];
                    } else {
                        $type = $val['type'];
                    }
                }
