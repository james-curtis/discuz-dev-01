...
class plugin_threed_attach
{
    ...
    { //预先把所有附件都先处理，使之不被解析，方便后面解析
        ...
        if (!in_array($_G['fid'], $pan_forum) || $thd_dazhe)
            return array();
...
class plugin_threed_attach_forum extends plugin_threed_attach
{
    ...
    function post_attach_btn_extra()
    {
        ...
        if (!$pid) {
            ...
            $iffirst = DB::result_first("SELECT first FROM " . DB::table('forum_post') .
                " WHERE pid='$pid' LIMIT 1");
        }
        ...
        if (!in_array($_G[groupid], $pan_fauser))
            return;
        if (!$thd_back && !$iffirst && $p_action == "edit")
            return;
        if (!$thd_back && $p_action == "reply")
            return;
        if (!in_array($_G['fid'], $pan_forum))
            return;
        $ndsvtreturn = "<li  id=\"e_btn_filelist_pan_upload\"><input type=\"hidden\" name=\"urlid\" value=\"CDF6307B-58E8-550B-8044-A330AD87BA1D\"><a href=\"javascript:;\" hidefocus=\"true\" onclick=\"switchAttachbutton('filelist_pan_upload');\">" .
            lang('plugin/threed_attach', 'admin1') . "</a></li><li  id=\"e_btn_filelist_pan_upload\"><a href=\"javascript:;\" hidefocus=\"true\" onclick=\" add_edit_button();\">" .
            lang('plugin/threed_attach', 'admin2') . "</a></li>";
        return $ndsvtreturn;
    }
    function post_attach_tab_extra()
    {
        ...
        $query = "SELECT v.groupid, v.grouptitle, t.readaccess FROM " . DB::table('common_usergroup') .
            " v LEFT JOIN " . DB::table('common_usergroup_field') .
            " t ON v.groupid=t.groupid where v.allowvisit=1 AND t.readaccess<>0 ORDER BY v.groupid DESC ";
        ...
        foreach ($postlist as $id => $post) {
            ...
            foreach ($post['attachments'] as $k => $attach) {
                if (!$attach['isimage']) {
                    ...
                    $pan_message = preg_replace("/\[oneattachatt\]" . $attach[aid] . "\[\/oneattachatt\]/",
                        $str, $pan_message);
                }
