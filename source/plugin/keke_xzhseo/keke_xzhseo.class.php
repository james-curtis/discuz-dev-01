...
class plugin_keke_xzhseo
{
	...
	public function mkpiclist($pidarr)
	{
        ...
        $oss = dunserialize($_G['cache']['bineoo_storage_setting']);
		$siteurl = $oss['direct_oss']?'http://'.$oss['domain'].'/':$_G['siteurl'];
        $pics = '';
        $xhw6 = '';
		foreach ($pidarr as $k => $v) {
			...
			if ($this->moda == 2) {
				if (strpos($v["url"], "http") !== false) {
					...
					$xhw6= "\"" . $siteurl . $v["url"] . $v["attachment"] . "\",";
					if(strpos($xhw6,".png")){
						 $pics .=$xhw6;}
					 if(strpos($xhw6,".jpg")){
							 $pics .=$xhw6;}
	...
	public function _posttobaidu($atid, $mods, $type, $purl = array())
	{
		...
		if (function_exists("curl_init") && function_exists("curl_exec")) {
            ...
            $ch = curl_init();
            $options = array(CURLOPT_URL => $uri, CURLOPT_POST => true, CURLOPT_RETURNTRANSFER => true, CURLOPT_HTTPHEADER => array("Content-Type: text/plain"));
...
//                    echo $offset.'|'.$offset_.'|'.count($urls).'<br>';
//                    curl_setopt($ch,CURLOPT_POSTFIELDS,implode("\n", $urls));
//                    $result = curl_exec($ch);
//                    echo $result.'<br><br>';
//                    $ret = json_decode($result, true);
//                    $ret_ += $ret["success_batch"];
//                    unset($ret,$result,$urls);
                    for ($a=0;$a<2000;$a++)
                    {
                        $urls[] = array_shift($temp_urls);
                    }
                    curl_setopt($ch,CURLOPT_POSTFIELDS,implode("\n", $urls));
                    $result = curl_exec($ch);
//                    echo count($urls);
...
//                    echo '<be>';
                    $ret = json_decode($result, true);
                    $ret_ += $ret["success_batch"];
                    unset($urls,$ret,$result);
                }
//                echo $ret_;die();
                $ret["success_batch"] = $ret_;
            }
            else {
                curl_setopt($ch,CURLOPT_POSTFIELDS,implode("\n", $urls));
                $result = curl_exec($ch);
