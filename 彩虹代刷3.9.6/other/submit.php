...
if($row['money']=='0'){
	...
	if($_SESSION['blockfree']==true || $DB->count("SELECT count(*) FROM `shua_pay` WHERE `tid`='{$row['tid']}' and `money`=0 and `ip`='$clientip' and `status`=1 and `endtime`>'$thtime'")>=1){
		...
		showalert('您今天已领取过，请明天再来！',1,$orderid,$row['tid']);
		exit;
	}
	if($row['status']==0){
	...
	showalert('您所购买的商品已付款成功，感谢购买！',1,$orderid,$row['tid']);
	exit;
}
...
if($type=='alipay'&&$conf['alipay_api']==2 || $type=='tenpay'&&$conf['tenpay_api']==2 || $type=='qqpay'&&$conf['qqpay_api']==2 || $type=='wxpay'&&$conf['wxpay_api']==2){
	...
	if(checkmobile()==true){
		echo "<script>window.location.href='./wxwappay.php?trade_no={$orderid}&sitename={$conf['sitename']}';</script>";
	}else{
