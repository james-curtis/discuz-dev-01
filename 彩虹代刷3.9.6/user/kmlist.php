...
$rs=$DB->query("SELECT * FROM shua_tools WHERE active=1 and cost!=0 order by sort asc");
$select='';
...
$id=intval($_GET['id']);
$kmrow=$DB->get_row("SELECT * FROM shua_kms WHERE kid='$id' limit 1");
$sql=$DB->query("DELETE FROM shua_kms WHERE kid='$id' and `zid`='{$userrow['zid']}'");
if($sql){
